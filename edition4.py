...
class visitor:
    ...
    def log_in(self, id, password):  # 系统登录  登录时候可以选择忘记密码
    ...
    def insertdb(self):
        self.cur.execute('INSERT INTO user(name,id,password,attribute,state,question,answer) VALUES (?,?,?,?,?,?,?)',
                         (self.name, self.id, self.password, "user", 0, self.question, self.answer))
        self.con.commit()

    def getsecretquestion(self,id):  # 获取id绑定的密保问题
        info = self.cur.execute(
            'SELECT * FROM user WHERE id = ?', (id,)).fetchone()
        return info[5]
    ...
    def panduid(self,s):
        info = self.cur.execute(
            'SELECT * FROM user WHERE id = ?', (s,)).fetchone()
        return info

    def update_newcode(self,id,code):
        self.cur.execute(
            'UPDATE user set password = ? where id = ?', (code, id,))
        self.con.commit()
    '''
    ...
    def get_noticetitle(self):  #获取公告标题
        path = os.getcwd()  # 当前工作路径
        curr_time = datetime.datetime.now().date()
        path = path + "\\Neu " + str(curr_time) + "\\Notice" + "\\标题.txt"
        with open(path, 'r+', encoding="gbk") as file:
    ...
    def result_time(self):  #获取时间排序帖子主题
    ...
    def result_hot(self):  #获取热度排序帖子主题
    ...
    def result_p(self,s):
        ...
        res1 = self.cur.execute(sql, (s,)).fetchall()
        # for j in res1:
        #     print(j)
        title = ["空","","","","","","","","",""]
        k = 0
    ...
    def chakantiezi(self,num):
        if num > len(self.pid):
            return "","",""
        post_id = self.pid[num-1]
        p = self.cur.execute('SELECT * from post where post_id = ?',(post_id,)).fetchone()
        content0 = p[-2]  # 标题
        content1 = p[-3] # 内容
        content2 = []  # 评论
        comment = self.cur.execute("SELECT reply from comment where post_id = ?", (post_id,))
        for i in comment:
            content2.append(i[0])
...
class adminstrator(visitor):
    ...
    def search_user(self):
        ...
        res = sql.fetchall()
        id = [(res.index(i) + 1, (res[0],res[1],res[2])) for i in res]
        return id
